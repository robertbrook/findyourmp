...
(Edited to remove references to the now defunct "experimetal" branch)
...
bc. <del>git checkout experimental
git pull origin experimental</del>

<del>or if this is first time use then</del>

bc. <del>git fetch origin experimental:experimental
...
6. Drop current postcodes and constituency data and, as they were based on the old data, the manually created and blacklisted postcodes as well

bc. mysql -uroot -p
truncate table postcodes;
truncate table constituencies;
truncate table blacklisted_postcodes;
truncate table manual_postcodes;
exit
...
bc. rake fymp:parse_new_postcodes oldfile=data/postcodes.txt newfile=data/pcd_pcon_aug_2009_uk_lu/pcd_pcon_aug_2009_uk_lu.txt output=data/new_postcodes.txt

10. Once we have a valid file, we need to rename the files so that they can be used in the next step of the process

bc. mv data/postcodes.txt data/postcodes_aug09.txt
cp data/new_postcodes.txt data/postcodes.txt
...
12. Setup the postcode district information and regenerate the friendly ids

bc. rake fymp:load_postcode_districts
rake friendly_id:make_slugs MODEL=Constituency

*Please note: The constituency.rb model was also updated during this process as it contained a number of methods that treated ons_id as an integer*
